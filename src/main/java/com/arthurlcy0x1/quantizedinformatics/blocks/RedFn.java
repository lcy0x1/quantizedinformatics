...
public class RedFn {
	...
	public static class Cont extends CTECont {
		...
		public Cont(int id, PlayerInventory inv) {
			this(id, inv, new Inventory(7), new IntArray(DATA_LEN));
		}

		protected Cont(int id, PlayerInventory inv, IInventory ent, IIntArray arr) {
			super(Registrar.CT_REDFN, id, inv, ent, 92);
	...
	public static class Scr extends CTEScr<Cont> {
		...
		private static final ResourceLocation GUI = new ResourceLocation(Registrar.MODID,
				"textures/gui/container/reduction_furnace.png");
		...
		protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
			...
			if (cont.burning()) {
				int k = cont.getRem();
				blit(i + 56, j + 62 + 12 - k, 176, 12 - k, 14, k + 1);
			}
	...
	public static class TE extends CTETE<TE> implements RedRecipe.Inv, ITickableTileEntity {
		...
		public void tick() {
			...
			if (inputCheck)
				rec = world.getRecipeManager().getRecipe(Registrar.RT_RED, this, world).orElse(null);
	...
	private static final ITextComponent TITLE = new TranslationTextComponent(
			"quantizedinformatics:container.reduction_furnace");

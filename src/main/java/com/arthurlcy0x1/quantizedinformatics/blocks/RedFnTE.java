...
import com.arthurlcy0x1.quantizedinformatics.recipe.RedInv;
...
public class RedFnTE extends TileEntity implements RedInv, INamedContainerProvider {

	private static final int[] SLOTS = { 0, 1, 2, 3, 4, 5 };

	private static final ITextComponent title = new TranslationTextComponent(
			"quantizedinformatics::container.reduction_furance");

	private final Inventory inv = new Inventory(7);

	public RedFnTE() {
		super(Registrar.TET_REDFN);
		LogManager.getLogger().warn("reduction furance entity active");
	}
	...
	public Container createMenu(int type, PlayerInventory inv, PlayerEntity ent) {
		return new RedFnCont(type, inv, this);
	}

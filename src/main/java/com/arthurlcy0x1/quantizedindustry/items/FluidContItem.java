...
public class FluidContItem extends Item {
	...
	public FluidContItem(Properties p, double storage) {
		super(p);
		this.addPropertyOverride(new ResourceLocation("fill"),
				(is, w, pl) -> (float) (1.0 * getStorage(is) / getMaxStorage(is)));
		this.addPropertyOverride(new ResourceLocation("type"), (is, w, pl) -> QuanFluid.getID(getFluid(is)));
		this.hasEffect(null);
		defMax = storage;
	}
